...
export class ControllerFactory {
  ...
  async addDefaults() {
    ...
    for (const controller of controllers) await controller.initialize(this._db);
  }
  ...
  get(type: string): Controller {
    ...
    if (!toRet) throw new Error(`Cannot find controller '${type}'`);
  ...
  private async create(type: string): Promise<Controller> {
    ...
    if (this._controllers[type]) return this._controllers[type];
