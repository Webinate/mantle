...
export class Schema<T extends IModelEntry<'server'>, Y extends IModelEntry<'client' | 'expanded'>> {
  ...
  public clone(clone?: Schema<T, Y>): Schema<T, Y> {
    ...
    for (let i = 0, l = items.length; i < l; i++) copy._items.push(items[i].clone());
  ...
  setServer(data: Partial<T>, allowReadOnlyValues: boolean) {
    ...
    for (const i in data) {
      for (const item of items)
        if (item.name === i && (allowReadOnlyValues || item.getReadOnly() === false)) item.setDbValue(data[i]);
    }
  }
  ...
  public uploadToken() {
    ...
    for (let i = 0, l = items.length; i < l; i++) toReturn[items[i].name as keyof T] = items[i].getDbValue();
  ...
  public async downloadToken(options?: Partial<ISchemaOptions>): Promise<Y> {
    ...
    for (let i = 0, l = items.length; i < l; i++) {
      ...
      // then skip the item
      if (items[i].getSensitive() && options.verbose === false) continue;
    ...
    for (let i = 0, l = returns.length; i < l; i++) toReturn[itemsInUse[i].name as keyof Y] = returns[i];
  ...
  public async validate(checkForRequiredFields: boolean) {
    ...
    for (let i = 0, l = items.length; i < l; i++) {
      ...
      if (items[i].autoValidate) promises.push(items[i].validate(clientVal || dbVal || defaultVal));
      else if (clientVal !== undefined) promises.push(items[i].validate(clientVal));
      else if (dbVal || dbVal === false) promises.push(dbVal);
      else promises.push(items[i].validate(defaultVal));
    }
    ...
    for (let i = 0, l = serverValues.length; i < l; i++) {
      ...
      if (this.dbEntry) this.dbEntry[items[i].name as keyof T] = serverValues[i];
    }
  ...
  public getByName<K extends keyof T>(val: K): SchemaItem<T[K], T[K]> {
    ...
    if (item === null) throw new Error(`Could not find '${val}'`);
  ...
  public find<K extends keyof T>(val: K): SchemaItem<T[K], T[K]> | null {
    const items = this._items;
    for (const item of items) if (item.name === val) return item;
  ...
  public addItems(items: SchemaItem<any, any>[]): SchemaItem<any, any>[] {
    for (const val of items) {
      if (val.name === '_id') throw new Error(`You cannot use the schema item name _id as its a reserved keyword`);
  ...
  public remove(val: SchemaItem<any, any> | string) {
    const items = this._items;
    let name = '';
    if (<SchemaItem<any, any>>val instanceof SchemaItem) name = (<SchemaItem<any, any>>val).name;
  ...
  uniqueFieldNames(): string {
    ...
    for (let i = 0, l = items.length; i < l; i++) if (items[i].getUnique()) uniqueNames += items[i].name + ', ';

    if (uniqueNames !== '') uniqueNames = uniqueNames.slice(0, uniqueNames.length - 2);
