  ...
  public async validate( val: string[] ) {
    ...
    for ( let i = 0, l = transformedValue.length; i < l; i++ ) {
      ...
      if ( transformedValue[ i ].length > maxCharacters )
        throw new Error( `The character length of '${transformedValue[ i ]}' in ${this.name} is too long, please keep it below ${maxCharacters}` );
      else if ( transformedValue[ i ].length < minCharacters )
        throw new Error( `The character length of '${transformedValue[ i ]}' in ${this.name} is too short, please keep it above ${minCharacters}` );
    }
