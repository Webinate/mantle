...
export class SchemaEnum extends SchemaItem<string, string> {
  ...
  public async validate(val: string | undefined) {
    ...
    if (!this.canBeEmpty && val!.trim() === '') throw new Error(`${this.name} cannot be `);
    else if (this.canBeEmpty && val!.trim() === '') return '';

    if (!enums.includes(val)) throw new Error(`${this.name} is not a valid entry`);
