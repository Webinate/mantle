...
export async function getAll() {
  const page = await getJson<Page<IDocument<'client'>>>( rootPath );
  return page;
}

export async function getOne( id: string ) {
  const doc = await getJson<IDocument<'client'>>( `${rootPath}/${id}` );
  return doc;
}

export async function setTemplate( docId: string, templateId: string ) {
  const doc = await putJson<IDocument<'client'>>( `${rootPath}/${docId}/set-template/${templateId}`, {} );
  return doc;
}
...
export async function editElement( docId: string, elementId: string, elm: Partial<IDraftElement<'client'>> ) {
  const doc = await putJson<IDraftElement<'client'>>( `${rootPath}/${docId}/elements/${elementId}`, elm );
  return doc;
}
