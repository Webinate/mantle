...
import { IDraft, IPopulatedDrfat } from './i-draft';
...
export interface IDocument<T extends 'server' | 'client'> {
  ...
  currentDraft: T extends 'client' ? IDraft<T> | IPopulatedDrfat<T> | null | string : ObjectID | null;
  publishedDraft: T extends 'client' ? IDraft<T> | IPopulatedDrfat<T> | string | null : ObjectID | null;
  lastUpdated: number;
  createdOn: number;
}