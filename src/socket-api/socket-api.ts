...
export class SocketAPI {
  ...
  private onMeta(e: ServerInstruction<any>) {
    ...
    if (!Factory.get('users')) return;

    Factory.get('users')
      .getUser(e.token.username!)
      .then(function(user) {
        if (!user) return Promise.reject(new Error('Could not find user ' + e.token.username));
        ...
        else if (e.token.property) return Factory.get('users').getMetaVal(id, e.token.property);
        else if (e.token.val) return Factory.get('users').setMeta(id, e.token.val);
        else return Factory.get('users').getMetaData(id);
      })
