...
describe( 'Testing registering a user', function() {
  ...
  it( 'should not register with blank credentials', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: "", password: "" } );
  ...
  it( 'should not register with existing username', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: header.admin.username, password: "FakePass" } );
    assert.deepEqual( resp.status, 500 );
    const json = await resp.json();
  ...
  it( 'should not register with blank username', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: "", password: "FakePass" } );
    assert.deepEqual( resp.status, 500 );
    const json = await resp.json();
    assert.deepEqual( json.message, "Please enter a valid username" )
  } )
  ...
  it( 'should not register with blank password', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: "sdfsdsdfsdfdf", password: "" } );
    assert.deepEqual( resp.status, 500 );
    const json = await resp.json();
  ...
  it( 'should not register with bad characters', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: "!\"�$%^^&&*()-=~#}{}", password: "!\"./<>;�$$%^&*()_+" } );
    assert.deepEqual( resp.status, 500 );
    const json = await resp.json();
  ...
  it( 'should not register with valid information but no email', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: testUserName, password: "Password" } );
    assert.deepEqual( resp.status, 500 );
    const json = await resp.json();
  ...
  it( 'should not register with valid information but invalid email', async function() {
    const resp = await header.admin.post( `/api/auth/register`, { username: testUserName, password: "Password", email: "bad_email" } );
    assert.deepEqual( resp.status, 500 );
    const json = await resp.json();
  ...
  it( 'did not approve activation as a regular user', async function() {
    const resp = await header.user1.put( `/api/auth/${testUserName}/approve-activation` );
  ...
  it( `did allow an admin to activate ${testUserName}`, async function() {
    const resp = await header.admin.put( `/api/auth/${testUserName}/approve-activation` );
    assert.deepEqual( resp.status, 200 );
  } )
