...
describe('Testing the adding of document elements: ', function() {
  ...
  it('did not add an element on a document that doesnt exist', async function() {
    const resp = await header.user1.post(`/api/documents/123456789012345678901234/elements`, {
      html: ''
  ...
  it('did not allow a guest to add an element', async function() {
    const resp = await header.guest.post(`/api/documents/${document._id}/elements`, { html: '' } as IDraftElement<
      'client'
    >);
  ...
  it('did not allow the creation of element without a type', async function() {
    ...
    const resp = await header.user1.post(`/api/documents/${document._id}/elements`, { html: html } as IDraftElement<
      'client'
    >);
  ...
  it('did not allow the creation of element without a valid type', async function() {
    const html = '<p>Hello world</p>';
    const resp = await header.user1.post(`/api/documents/${document._id}/elements`, {
      type: 'BAD' as any,
      html: html
    } as IDraftElement<'client'>);
    assert.equal(resp.status, 400);
