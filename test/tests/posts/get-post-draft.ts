...
describe('Testing of posts and drafts', function() {
  ...
  it('prevents getting post draft lists with a bad id', async function() {
    ...
    assert.deepEqual(
      errors![0].message,
      'Variable "$id" got invalid value "BAD"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer'
    );
  });
  ...
  it('prevents removing a post draft with a bad id', async function() {
    ...
    assert.deepEqual(
      errors![0].message,
      'Variable "$draftId" got invalid value "BAD"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer'
    );
    assert.deepEqual(
      errors![1].message,
      'Variable "$postId" got invalid value "BAD"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer'
    );
  });
