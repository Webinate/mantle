...
describe('[GQL] Testing the fetching of documents: ', function() {
  ...
  //   const { data: doc } = await header.user1.graphql<IDocument<'expanded'>>(GET_DOCUMENT, { id: post.document });
  //   assert.deepEqual(doc._id, post.document.toString());
  // });

  it('did not get a document for a post when not the author', async function() {
    const { errors } = await header.user2.graphql<IDocument<'expanded'>>(GET_DOCUMENT, { id: post.document });
