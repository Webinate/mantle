...
describe( '8. Getting uploaded user files', function() {
  ...
  it( 'regular user did not get files for the admin user bucket', async function() {
    const resp = await header.user1.get( `/files/users/${( header.config.adminUser as IAdminUser ).username}/buckets/BAD_ENTRY` );
    const json = await resp.json();
  ...
  it( 'regular user did not get files for a bucket with bad id', async function() {
    const resp = await header.user1.get( `/files/users/${header.user1.username}/buckets/test` );
    const json = await resp.json();
  ...
  it( 'regular user did not get files for a non existant bucket', async function() {
    const resp = await header.user1.get( `/files/users/${header.user1.username}/buckets/123456789012345678901234` );
    const json = await resp.json();
    assert.deepEqual( resp.status, 500 );
  ...
  it( 'admin fetched 2 files from the regular users dinosaur bucket', async function() {
    const resp = await header.admin.get( `/files/users/${header.user1.username}/buckets/${bucket}` );
    const json = await resp.json();
    assert.deepEqual( resp.status, 200 );
    assert( json.data.length === 2 );
  } )
