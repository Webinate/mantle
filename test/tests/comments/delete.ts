...
describe('Testing deletion of comments', function() {
  ...
  it('cannot delete a comment with a bad id', async function() {
    ...
    assert.deepEqual(
      errors![0].message,
      `Variable "$id" got invalid value "abc"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer`
    );
  });
