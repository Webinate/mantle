...
describe('Testing creation of comments', function() {
  ...
  it('cannot create a comment with a badly formatted post id', async function() {
    ...
    assert.deepEqual(
      ...
      'Variable "$token" got invalid value "bad" at "token.post"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer'
    );
    assert.deepEqual(
      resp.errors![1].message,
      'Variable "$token" got invalid value "bad" at "token.parent"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer'
    );
  });
