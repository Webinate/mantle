...
describe('Testing file renaming', function() {
  ...
  it('regular user did not rename an incorrect file to testy', async function() {
    ...
    assert.deepEqual(
      ...
      'Variable "$token" got invalid value "123" at "token._id"; Expected type "ObjectId". Argument passed in must be a string of 12 bytes or a string of 24 hex characters or an integer'
    );
  });
