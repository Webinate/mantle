...
describe('[GQL] Testing registering a user', function() {
  ...
  it('[GQL] did not approve activation as a guest', async function() {
    const resp = await header.guest.graphql<{ approveActivation: boolean }>(
      `mutation { approveActivation(username:"${testUserName}") }`
    );
  ...
  it('[GQL] did not approve activation as a regular user', async function() {
    const resp = await header.user1.graphql<{ approveActivation: boolean }>(
      `mutation { approveActivation(username:"${testUserName}") }`
    );
  ...
  it('[GQL] did allow an admin to activate ${testUserName}', async function() {
    const resp = await header.admin.graphql<{ approveActivation: boolean }>(
      `mutation { approveActivation(username:"${testUserName}") }`
    );
    assert.deepEqual(resp.data.approveActivation, true);
  });
