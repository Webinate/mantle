...
describe( '1. Testing creation of categories', function() {
  ...
  it( 'did not create a category without a title', async function() {
    const resp = await header.admin.post( `/api/categories`, { slug: 'test' } as ICategory );
    assert.equal( resp.status, 500 );
    assert.equal( resp.statusText, `title cannot be empty` );
  } )

  it( 'did not create a category with an empty title', async function() {
    const resp = await header.admin.post( `/api/categories`, { title: '', slug: 'test' } as ICategory );
    assert.equal( resp.status, 500 );
    assert.equal( resp.statusText, `title cannot be empty` );
  } )

  it( 'did not create a category with an empty slug', async function() {
    const resp = await header.admin.post( `/api/categories`, { title: 'Test', slug: '' } as ICategory );
    assert.equal( resp.status, 500 );
    assert.equal( resp.statusText, `slug cannot be empty` );
  } )
  ...
  it( 'did create category called _test with no html', async function() {
    ...
    category = await header.admin.getJson( `/api/categories/slug/_test` );
    if ( category )
